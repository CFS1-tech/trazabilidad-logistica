        ...
        if not df_pl.empty:
            ...
            with st.form("f_ing"):
                ...
                if st.form_submit_button("Confirmar Ingreso"):
                    ...
                    st.success(f"✅ Registrado: SKU {s_sel} en {c_sel} (Vence: {fv_s})")

# --- 2. SECCIÓN REPORTE DE STOCK ---
...
# --- 3. SECCIÓN REPORTE DE MERMA (DESDE MOVIMIENTOS) ---
elif menu == "🗑️ Reporte de Merma":
    st.header("🗑️ Registro Histórico de Mermas")
    st.info("Este reporte analiza la hoja de Movimientos para mostrar todos los ingresos marcados como 'Merma'.")
    df_m = pd.DataFrame(ws_mov.get_all_records())
    if not df_m.empty:
        df_m.columns = df_m.columns.str.strip().str.lower()
        
        # Limpieza y estandarización para búsqueda robusta
        ...
        with st.container(border=True):
            ...
            f_cont_m = c1.multiselect("Seleccione Contenedores:", sorted(df_m['contenedor'].unique().astype(str)))
            btn_m = c2.button("🔍 Consultar Historial de Merma")
            
            if btn_m:
                # Filtrar por estado 'merma' en ingresos
                ...
                if not res_m.empty:
                    st.warning(f"📊 Total histórico de merma registrado: {int(res_m['cantidad'].sum())} unidades.")
                    res_m = formatear_fecha_lectura(res_m, 'fecha_hora')
                    st.dataframe(res_m[['fecha_hora_fmt', 'sku', 'contenedor', 'cantidad', 'referencia']].rename(columns={'fecha_hora_fmt': 'fecha_registro'}), use_container_width=True)
                else:
                    st.error("No se encontraron registros históricos de merma para los filtros seleccionados.")

# --- 4. SECCIÓN HISTORIAL ---
elif menu == "📊 Historial":
    st.header("📊 Trazabilidad Completa")
    df_m = pd.DataFrame(ws_mov.get_all_records())
    if not df_m.empty:
        df_m.columns = df_m.columns.str.strip().str.lower()
...
# --- 5. SECCIÓN PACKING LIST ---
elif menu == "📋 Packing List":
    st.header("📋 Cruce de Información vs Packing List")
    df_pl = pd.DataFrame(ws_pl.get_all_records())
    df_mov = pd.DataFrame(ws_mov.get_all_records())
    if not df_pl.empty:
        df_pl.columns = df_pl.columns.str.strip().str.lower()
        df_mov.columns = df_mov.columns.str.strip().str.lower()
        cont_f = st.selectbox("Seleccione Contenedor:", ["Todos"] + sorted(df_pl['contenedor'].unique().astype(str)))
        if st.button("🔍 Calcular Diferencias"):
            # Lógica de suma de ingresos reales
            ...
            # Unión con Packing List original
...
# --- 6. SECCIÓN INSIGHTS ---
    ...
    if not df_i.empty:
        ...
        if not prox.empty:
            ...
            st.success("No hay productos próximos a vencer.")
        ...
        with c2:
            st.subheader("Ocupación por Contenedor (Top 10)")
